...
set "YOLO_URL=https://github.com/ultralytics/assets/releases/download/v8.3.0/%YOLO_MODEL%.pt"
...
:: -----------------------------------------------
set /a STEP+=1
if exist "%DEPS%\models\%YOLO_MODEL%.onnx" (
    echo [%STEP%/5] %YOLO_MODEL%.onnx - already present, skipping
    goto :skip_yolo
)
echo [%STEP%/5] Downloading %YOLO_MODEL% and exporting to ONNX...
echo      This requires ultralytics: pip install ultralytics
echo.

:: Check if the .pt file exists (may have been downloaded manually)
if exist "%DEPS%\models\%YOLO_MODEL%.pt" (
    echo      Found %YOLO_MODEL%.pt, exporting to ONNX...
    goto :export_onnx
)

:: Download the .pt model
echo      Downloading %YOLO_MODEL%.pt...
curl -L -o "%DEPS%\models\%YOLO_MODEL%.pt" "%YOLO_URL%"
if errorlevel 1 (
    echo ERROR: Failed to download YOLO model
    del "%DEPS%\models\%YOLO_MODEL%.pt" 2>nul
    exit /b 1
)

:export_onnx
:: Export to ONNX using ultralytics
python -c "from ultralytics import YOLO; model = YOLO(r'%DEPS%\models\%YOLO_MODEL%.pt'); model.export(format='onnx', imgsz=640, simplify=True, opset=17)"
if errorlevel 1 (
    echo.
    echo ERROR: ONNX export failed. Make sure ultralytics is installed:
    echo        pip install ultralytics
    echo.
    echo Alternatively, export manually and place %YOLO_MODEL%.onnx in dependencies\models\
    exit /b 1
)

:: Move the exported .onnx file (ultralytics puts it next to the .pt)
if exist "%DEPS%\models\%YOLO_MODEL%.onnx" (
...
if not exist "%DEPS%\models\%YOLO_MODEL%.onnx" set "MISSING=!MISSING! models/%YOLO_MODEL%.onnx"
if defined MISSING (
    ...
    echo   dependencies\nssm.exe
    echo   dependencies\models\%YOLO_MODEL%.onnx
)
echo.
