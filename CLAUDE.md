...
20. Bulk Camera Add Wizard - "Add Camera" button now opens a chooser: **Scan network** or **Enter manually**. The scan wizard (`app/lib/screens/add_cameras_wizard_screen.dart`) is a 5-step flow: credentials → LAN scan → RTSP URL resolve → name + confirm → batch create. Backend provides two new endpoints: `POST /api/cameras/scan` auto-detects private /24 subnets from the server (via `socket.getaddrinfo(socket.gethostname())`), TCP-probes port 554 with bounded concurrency (64), sends RTSP OPTIONS for a Server-header brand hint, and excludes IPs already attached to existing cameras. `POST /api/cameras/discover_batch` is a parallel wrapper around the existing per-IP `/discover` logic (host_concurrency=4) that runs the 58 brand-pattern probe for multiple hosts at once. Wizard auto-picks the highest-resolution match per host, falls back to manual URL entry when no pattern matches, and sequentially creates cameras at the end to keep stream-manager reloads safe. Zero new Python/Dart dependencies. (DONE)
