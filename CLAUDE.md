...
- **Playback**: Recordings are HEVC .ts files. Direct = instant remux (`-c copy`), High/Low = H.264 NVENC transcode with source-probed bitrate (2x for HEVC). All use fragmented MP4 (`-movflags frag_keyframe+empty_moov`) streamed via StreamingResponse — playback starts in ~200ms. Single files use `-ss` before `-i` for fast keyframe seek. Multiple files use concat demuxer with `-ss` after `-i`. Sessions auto-cleanup after 120s idle.
...
- **Bitrate probing**: At startup, backend probes each camera's actual bitrate (5s ffmpeg sample) and codec (ffprobe). HEVC→H.264 conversion uses 2x bitrate multiplier for equivalent visual quality. Playback also probes .ts file bitrate+codec via ffprobe before transcoding.
- **Live streams**: go2rtc registers 6 streams per camera (Main/Sub × Direct/High/Low). Unused quality streams are lazy — zero resources until a client connects.
  ...
  - Main High: native res H.264, source-matched quality (2x bitrate for HEVC→H.264 conversion, probed at startup via ffmpeg sampling)
  - Main Low: native res H.264, reduced quality (1/4 of High bitrate)
  - Sub Direct: raw sub-stream passthrough (no ffmpeg)
  - Sub High: native res H.264, source-matched quality (2x bitrate for HEVC sources)
  - Sub Low: native res H.264, reduced quality (1/4 of High bitrate)
- **Playback**: All qualities use fragmented MP4 streaming. Direct = HEVC passthrough (`-c copy`), High = H.264 NVENC source-matched quality (probed from .ts file, 2x for HEVC), Low = H.264 NVENC reduced quality (1/4 of High).
...
- **Fullscreen page**: Full video player + timeline + speed controls (shown in playback mode). Click timeline segment → instant MP4 remux (< 1s) → video plays via media_kit. Video stats bar (codec, resolution, FPS, bitrate) togglable via icon button. Bug report button in header.
