...
- **Client installer**: `ISCC.exe installer\richiris_client.iss` → `dist/RichIris-Client-Setup-1.0.0.exe` (Flutter app + VC redist only — no backend/service/firewall/go2rtc/ffmpeg). Ships `client_only.txt` marker; coexists with the full installer on the same PC.
