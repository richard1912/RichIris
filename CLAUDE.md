...
- **Full installer**: `ISCC.exe installer\richiris.iss` → `dist/RichIris-Setup-1.0.0.exe` (backend + service + Flutter app, downloads deps at install time)
- **Client-only installer**: `ISCC.exe installer\richiris_client.iss` → `dist/RichIris-Client-Only-Setup-1.0.0.exe` (Flutter app + VC redist only — no backend/service/firewall/go2rtc/ffmpeg). Shipped as the alternative to the full installer — **end users install one or the other, not both**. Ships `client_only.txt` marker; coexists with the full installer on the same PC if a dev happens to have both. Update detection keys on the substring `"Client-Only"` in the asset filename (both `update_checker.py` and `update_service.dart`).
- **Release script**: `push_release.bat` (builds full Windows installer + client Windows installer + Android APK, creates GitHub release with Claude-generated changelog, gitignored)
...
Camera RTSP (main) ← go2rtc (:18700/:18554) (s1 keepalive) ──→ live view clients (RTSP via :18554)
                                               ──→ transcoded quality variants (lazy)

Camera RTSP (sub)  ← go2rtc (:18700/:18554) ← FrameBroker (persistent ffmpeg, MJPEG @ 2fps)
                                                │──→ motion detection (shared buffer)
                                                │──→ thumbnail capture (shared buffer)
...
- **go2rtc keepalives for instant live view**: StreamManager runs a single in-process httpx HTTP fMP4 consumer per camera (`s1_direct` only) that reads from go2rtc's HTTP API (`/api/stream.mp4`) and discards data. Keeps the main-stream RTSP connection warm for instant main-quality live view. The sub-stream (`s2_direct`) is kept warm by the FrameBroker's persistent ffmpeg reader instead — no separate keepalive needed. Keepalives start staggered (1s apart per camera). Auto-reconnects on failure with 5s retry. Transcoded quality variants (high/low/ultralow) chain off the direct stream within go2rtc. If no sub-stream URL is configured, s2_direct chains off s1_direct within go2rtc.
- **FrameBroker**: Shared service (`app/services/frame_broker.py`) that runs one persistent ffmpeg process per camera pulling MJPEG frames from go2rtc's local `s2_direct` relay (`rtsp://127.0.0.1:18554/{stream}_s2_direct`) at 2 fps. Stdout is parsed via JPEG SOI/EOI markers into numpy frames stored in a per-camera latest-frame buffer. Consumers (motion detector, thumbnail capture) read via `get_latest(camera_id)` (instant, may be up to 4 s stale) or `get_fresh(camera_id, max_wait)` (awaits a new frame). This replaces the previous "spawn ffmpeg per frame" pattern, which cost 2–4 s per snapshot due to RTSP handshake + keyframe wait. No additional camera connections — piggybacks on go2rtc's existing sub-stream. Also serves as the sub-stream keepalive (go2rtc sees a permanent consumer). Auto-restarts on ffmpeg exit with 3 s backoff. Frame broker starts before motion + thumbnail capture during app lifespan startup.
...
│   │   ├── services/            # API layer (Dio HTTP client) + settings_api.dart + backup_api.dart + update_service.dart + timeline_cache.dart
│   │   ├── screens/             # Home, Fullscreen, System, Settings (unified), CameraForm
│   │   ├── widgets/             # CameraGrid, CameraCard, LivePlayer, QualitySelector, DateTimePickerDialog, ExportClipWizardDialog, StorageMigrationDialog, BackupRestoreDialog, UpdateDialog, VersionInfoDialog, bug_report_dialog.dart
...
- `POST /api/cameras/scan` - LAN-scan for IP cameras. Auto-detects server's private /24 subnets (or accepts `{subnets: ["192.168.8"]}`), TCP-probes port 554 with bounded concurrency, sends RTSP OPTIONS for a brand hint via the Server header. Excludes IPs already attached to an existing camera. Returns `{subnets_scanned, hosts_probed, hits: [{ip, port, server_header, brand_hint}], elapsed_ms}`. Used by the "Scan & Add" wizard in the Flutter app.
- `POST /api/cameras/discover_batch` - Parallel wrapper around `/discover` for many hosts: `{targets: [{ip, username, password, port}], host_concurrency}`. Returns `{results: {ip: [RtspDiscoverResult, ...]}}`. Used by the wizard after the user picks hosts from the scan.
- `POST /api/cameras/snapshot` - Grab a single JPEG frame from an arbitrary RTSP URL: `{rtsp_url, width?, timeout_s?}`. Returns `image/jpeg` bytes (downscaled to `width`, default 320). Used by the Scan & Add wizard's name step to show a preview thumbnail next to each camera so users can identify them visually before choosing a name. Not tied to a stored camera — caller provides the full URL with credentials embedded. 504 on ffmpeg timeout, 502 on ffmpeg failure.
...
- `GET/POST/PUT/DELETE /api/cameras` - Camera CRUD. `DELETE /api/cameras/{id}?purge_data=true` permanently deletes the camera's recording files, thumbnails, and motion event data from disk; without the parameter, files are preserved (DB rows are always deleted).
...
- **Fullscreen page**: Full video player + timeline + speed controls (shown in playback mode). Click timeline segment → plays via media_kit (Direct = raw .ts, other qualities = NVENC transcode). Video stats bar (codec, resolution, FPS, bitrate) shown by default, togglable via icon button. Refresh button (next to Stream/Quality selectors) stops + reopens RTSP (live) or restarts transcode session at current position (playback). Bug report button in header.
...
19. Client-only Windows installer + LAN auto-discovery - Second Inno Setup script (`installer\richiris_client.iss`) ships only the Flutter Windows app + VC redist DLLs (no backend/service/firewall/go2rtc/ffmpeg/data dir) for users on a second PC. Marks itself with an empty `client_only.txt` next to the exe. First-run `SettingsScreen` has a **Scan network** button that probes every private /24 subnet reachable via `NetworkInterface.list` for `/api/health` responses containing `{"app": "richiris"}`, letting users pick their backend from a list without typing an IP (built-in `dart:io` `HttpClient`, no new packages). `/api/health` extended with `app` + `version` fields for scanner identification. Auto-updater is flavor-aware: backend `update_checker.py` splits `.exe` assets into `assets["windows"]` (full, stays as-is for back-compat) and `assets["windows_client"]` (contains "Client-Setup" in filename). `UpdateInfo` carries both URLs; `UpdateService.downloadUpdate` picks the right one via `isClientOnlyInstall()`. Client-only installs have no backend to broker the GitHub check, so `UpdateService.getUpdate`/`checkNow` hit `https://api.github.com/repos/richard1912/RichIris/releases` directly and synthesize an `UpdateInfo` client-side (mirrors `update_checker.py` parsing). Full installs keep using the backend's cached result. (DONE)
20. Bulk Camera Add Wizard - "Add Camera" button now opens a chooser: **Scan network** or **Enter manually**. The scan wizard (`app/lib/screens/add_cameras_wizard_screen.dart`) is a 5-step flow: credentials (+ optional subnet override for NAT'd environments) → LAN scan → RTSP URL resolve → name + confirm (with live preview thumbnails) → batch create. Backend provides three new endpoints: `POST /api/cameras/scan` auto-detects private /24 subnets from the server (via `socket.getaddrinfo(socket.gethostname())` — or accepts an explicit `subnets` list), TCP-probes port 554 with bounded concurrency (64), sends RTSP OPTIONS for a Server-header brand hint, and excludes IPs already attached to existing cameras. `POST /api/cameras/discover_batch` is a parallel wrapper around the existing per-IP `/discover` logic (host_concurrency=4) that runs the 58 brand-pattern probe for multiple hosts at once. `POST /api/cameras/snapshot` grabs a single JPEG frame from a given RTSP URL via `ffmpeg -frames:v 1` (downscaled to 320px wide, 8 s timeout) so the wizard can show a visual preview next to each pending camera on the name step — this dramatically helps users identify which physical camera is which before naming. Thumbnails are fetched from the wizard with a small client-side worker pool (3 concurrent) as the user enters step 3. Wizard auto-picks the highest-resolution match per host, falls back to manual URL entry when no pattern matches, and sequentially creates cameras at the end to keep stream-manager reloads safe. Zero new Python/Dart dependencies. (DONE)
21. Timeline cache + bug report filter - New `TimelineCache` service (`app/lib/services/timeline_cache.dart`) prewarms today's recording segments, motion events, and trickplay thumbnails for every enabled camera at app startup (3 concurrent camera workers). Eliminates multi-second blocking delay on first timeline click. Cache persists across navigations and auto-sweeps stale (pre-today) entries at midnight rollover and when cameras are deleted. TimelineWidget reads from cache on initState/didUpdateWidget and writes back fetched data for next-app-session hit. Image precaching of latest 5 thumbnails per camera via `precacheImage` so timeline hover returns instantly. Shared `showBugReportDialog` widget (`app/lib/widgets/bug_report_dialog.dart`) replaces duplicate dialogs in home + fullscreen screens. Strips ANSI color codes, extracts `app.xxx.yyy` logger names, exposes a module dropdown + free-text "Contains" AND filter. Copy button copies only filtered view. Header shows filtered/total line counts. LivePlayer defer fix: moved `_open()` from initState into post-frame callback to prevent setState-during-build assert in debug mode. (DONE)
22. Refresh feed button + client-event logging + focus-theft fix - IconButton next to Stream/Quality selectors in both grid and fullscreen views. **Grid refresh now refreshes ALL enabled cameras** (not just selected one) in both live and playback modes — live mode iterates all enabled cameras' players and does stop+reopen on each; playback mode uses `_startPlayback(currentIso)` which already restarts all cameras' sessions. Button always enabled with tooltip "Refresh all feeds". Fullscreen refresh always enabled for the active camera. **Focus-theft fix**: Moved `KeyboardListener`'s FocusNode from `build()` to state with requestFocus in post-frame initState callback, preventing stats timer's 1s setState from re-stealing focus from active TextFields (bug report dialog couldn't be typed in). **New `POST /api/system/client-event` endpoint**: lets Flutter app log client-side breadcrumbs (event name + details dict) to backend structured log for "Report a Bug" log viewer surfacing. Both grid and fullscreen `_refreshFeed()` methods now fire `logClientEvent(event: 'refresh_feed', details: {...})` on every press. (DONE)
23. Settings cache prewarm + data dir always visible + camera delete with purge - `SettingsApi.prewarm()` fires at app startup (alongside timeline cache) and populates `cachedSettings` + `cachedDataDir` fields from the backend. `SystemSettingsScreen._load()` hydrates UI from cache immediately (no spinner) then refreshes in background; cache auto-invalidates on `updateSettings()`. Data directory section now always renders in Settings (previously hidden on fetch failure). `DELETE /api/cameras/{id}?purge_data=true` now permanently deletes camera recording/thumbnail/detection directories via `shutil.rmtree`; UI shows a confirmation dialog with checkbox "Also delete all recordings, thumbnails, and detection data" (unchecked = preserve files). (DONE)
