...
22. Refresh feed button - IconButton next to Stream/Quality selectors in both grid and fullscreen views. Live mode: calls `player.stop()` + `player.open(Media(url))` to fully tear down and reconnect RTSP socket. Playback mode: reads current scrub position and restarts transcode session at that timestamp. Grid applies to selected camera (button disabled if no selection) or all cameras in grid playback. Fullscreen always enabled for the active camera. Debug logging: `[REFRESH] {grid|fullscreen} {live|playback} ...`. (DONE)
